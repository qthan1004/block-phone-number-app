...
const CallLogItem = ({ item }: { item: CallLogEntry }) => {
  ...
  const bgColor = isBlocked ? 'rgba(255, 90, 95, 0.1)' : 'rgba(46, 210, 153, 0.1)';
  const badgeBg = isBlocked ? 'rgba(255, 90, 95, 0.15)' : 'rgba(46, 210, 153, 0.15)';
    ...
    <Surface style={styles.card} elevation={1}>
      <View style={styles.cardHeader}>
        ...
        <View style={styles.cardRight}>
          <View style={[styles.badge, { backgroundColor: badgeBg, borderColor: color + '33' }]}>
...
export const CallLogScreen = () => {
  ...
  const handleClear = () => {
    Alert.alert('Xoá Lịch Sử', 'Bạn có chắc muốn xoá toàn bộ lịch sử cuộc gọi chặn?', [
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        {callLogs.length > 0 && (
          <TouchableOpacity style={styles.filterBtn} onPress={handleClear} activeOpacity={0.7}>
            <MaterialCommunityIcons name="delete-sweep" size={24} color="#FF5A5F" />
          </TouchableOpacity>
        )}
      </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <View style={styles.emptyIconCircle}>
              <MaterialCommunityIcons name="history" size={48} color="rgba(141, 147, 165, 0.5)" />
            </View>
            <Text style={styles.emptyTitle}>Chưa có lịch sử cuộc gọi</Text>
            <Text style={styles.emptySubtitle}>Khiên chặn rác đang hoạt động theo dõi.</Text>
          </View>
        }
      />
    </View>
